...
impl RecorderProcessor {
        ...
        P: OutputPin,
    {
        ...
        for _ in 0.._cout {
            ...
            let _ = led.set_state(config::LED_ENABLE.into());
            CurrentTask::delay(Duration::ms(_period.to_ms() / 2));
            let _ = led.set_state(config::LED_DISABLE.into());
            CurrentTask::delay(Duration::ms(_period.to_ms() / 2));
        }
        led
    }
        ...
        P: OutputPin,
    {
        ...
        loop {
            ...
            let _ = _led.set_state(crate::config::LED_ENABLE.into());
            ...
            #[cfg(feature = "led-blink-each-block")]
            let _ = _led.set_state(crate::config::LED_DISABLE.into());
