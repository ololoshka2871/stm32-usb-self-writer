...
| PA5  | F_TEMP1                 | Вход частоты давления (TIM2_CH1)  Собственно температура |
| PB14 | F_TEMP2                 | Вход частоты давления (TIM15_CH1) Компенсация            |
|      | **Аналоговые входы**    |                                                          |
| PA1  | ADC_BAT (ADC_5)         | Вход напряжения батареи (Делитель 100k/91k)              |
|      | **Питание генераторов** |                                                          |
| PB5  | EN_PRES                 | Питание генератора частоты давления                      |
| PB6  | EN_TEMP1                | Питание генератора частоты температуры 1                 |
| PB2  | EN_TEMP2                | Питание генератора частоты температуры 2                 |
|      | **Индикация**           |                                                          |
| PA10 | LED                     | Светодиод индикации (вкл. лог.0)                         |
|      | **QSPI Flash**          |                                                          |
| PA3  | CLK                     | Тактовый сигнал на оба канала                            |
| PA0  | RESET                   | Сброс флешки                                             |
...
Подключиться к устройству с помощью `openocd -f openocd.cfg`. Далее настроекны скрипты в VSCode.

## Принцип действия

Переключение между режимами происходит автоматически в зависимости от способа подключения устройства, запрещается одновременное подключение по USB и к батарее!

### Режим датчика

При подлклчении устройства по USB оно определяется как USB-накопитель + витруальный COM-порт.
Устройство преднозначено для работы с KalibratorGUI, который может настраивать устройство и и получать текущие данные. В этом режиме производится градуировка устройства.

На виртуальном USB-накопителе присутствуют файлы при помощи которых пользователь может читать записанные самописцем данные, см. ниже.

### Режим самописца

При подключении к батарее устройство переходит в режим самописца, в котором оно начинает собирать данные с частотных входов и записывать их во внутреннюю Flash-память.
